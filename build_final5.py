...
OUT = BASE / "enceladus_final_v5_9x16.mp4"
...
# ---------------------------------------------------------------- captions v3
SCIENCE = {"ocean", "geysers", "cassini", "spray", "salts", "organic", "silica",
           "phosphates", "life", "ring", "water", "vents", "ice", "saturn",
           "spacecraft", "hydrothermal"}
...
def build_s7(d7):
    ...
    src = Image.open(IMGR / "PIA17216.jpg").convert("RGB")
    side = min(src.size)
...
def build_s8(d8):
    ...
    center("FOLLOW FOR MORE COSMIC DISCOVERIES", f1, (240, 237, 230), 1250)
    center("COMMENT: WHAT IS HIDING DOWN THERE?", f2, (143, 216, 255), 1340)
    p = subprocess.Popen(["ffmpeg", "-y", "-loglevel", "error", "-f", "rawvideo",
                          "-pix_fmt", "rgb24", "-s", f"{W}x{H}", "-r", str(FPS), "-i", "-",
                          "-vf", "fade=t=in:st=0:d=0.5,fade=t=out:st=3.7:d=0.5",
                          "-c:v", "libx264", "-preset", "fast", "-crf", "19", "-an",
...
def main():
    ...
    SHOTS = {
        "s1": [("PIA17216", "punch_in", 0, 0.16, 0),
               ("PIA14642", "crash_zoom", 2.5, 0.42, 0),
               ("PIA06443", "crash_zoom", 2.0, 0.46, 0),
               ("PIA17202", "punch_in", 0, 0.34, 0)],
        "s2": [("PIA14642", "punch_in", 0, 0.30, 0),
               ("PIA08354", "crash_zoom", 2.0, 0.30, 0),
               ("PIA06205", "crash_zoom", 2.5, 0.44, 0),
               ("PIA17198", "whip_pan", 1.5, 0.30, 0),
               ("PIA17205", "punch_out", 0, 0.30, 0)],
        "s3": [("PIA17216", "punch_in", 0, 0.18, 0),
               ("PIA10352", "crash_zoom", 2.5, 0.46, 0),
               ("PIA11688", "whip_pan", 1.5, 0.38, 0),
               ("PIA13620", "crash_zoom", 2.0, 0.52, 0),
               ("PIA11686", "punch_out", 0, 0.30, 0)],
        "s4": [("PIA08921", "punch_in", 0, 0.20, 0),
               ("PIA12512", "whip_pan", 1.5, 0.36, 0),
               ("PIA17172", "drift", 0, 0.28, 0),
               ("PIA08133", "punch_out", 0, 0.32, 0),
               ("PIA12512", "crash_zoom", 2.0, 0.42, 0)],
        # fix 6: diagram shots framed past legend bands (yoff pushes text bands out)
        "s5": [("PIA17216", "punch_in", 0, 0.18, 0),
               ("PIA08354", "crash_zoom", 2.0, 0.34, 0),
               ("PIA18071", "punch_in", 0, 0.34, 0.22),
               ("PIA18071", "crash_zoom", 2.5, 0.55, 0.34),
               ("PIA18071", "crash_zoom", 2.0, 0.74, 0.52),
               ("PIA18071", "drift", 0, 0.82, 0.72),
               ("PIA14937", "whip_pan", 1.5, 0.46, 0.14)],
        "s6": [("PIA17216", "punch_out", 0, 0.30, 0),
    ...
    for si, k in enumerate(order):
        ...
        CHIPS = {
            ...
            "s6": [hud_chip("BEST PLACE TO SEARCH FOR LIFE", 2.0, 6.5)],
        }
        vf += CHIPS.get(k, [])
        ids = PIA_IDS[k]
    ...
    # end cards
    d7 = max(6.5, segs["s7"]["duration"] + 1.6)
    d8 = 4.2
    build_s7(d7)
    build_s8(d8)
